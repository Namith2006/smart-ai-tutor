...
function App() {
  ...
  const handleStartProcess = async () => {
    ...
    try {
      if (inputType === 'text') {
        ...
        setLoadingMessage("Checking syllabus relevance...");
  ...
  const startAdaptiveSession = async () => {
    ...
    try {
      const res = await axios.post('http://127.0.0.1:8000/api/generate-session/', { content: inputText, mode: 'adaptive', preferences: [] });
    ...
    <div style={{ padding: '40px', maxWidth: '900px', margin: '0 auto', fontFamily: 'sans-serif', color: '#333' }}>
        ...
        <div style={{ background: '#f8f9fa', padding: '30px', borderRadius: '10px', animation: 'fadeIn 0.5s' }}>
          ...
          {inputType === 'topic' && (
            <div style={{ animation: 'fadeIn 0.5s' }}>
              ...
              <h3 style={{ marginTop: '10px' }}>Personalize your content:</h3>
      ...
      {currentStep === 'quiz' && sessionData && !loading && (
        <div style={{ animation: 'fadeIn 0.5s' }}>
          ...
          <h3 style={{ borderTop: '2px solid #444', paddingTop: '20px', color: '#f8f9fa' }}>
          ...
          <div style={{ background: '#fff', padding: '20px', border: '1px solid #eee', borderRadius: '10px', boxShadow: '0 4px 6px rgba(0,0,0,0.05)' }}>
            <p style={{ fontSize: '1.2rem', fontWeight: 'bold', color: '#111' }}>
              {sessionData.quiz[answeredCount].question}
            </p>
            ...
            {sessionData.quiz[answeredCount].options?.map((opt, i) => {
              ...
              return (
                <button 
                  ...
                  onClick={() => handleAnswerSubmit(opt, sessionData.quiz[answeredCount].correct_answer, sessionData.quiz[answeredCount].topic_tag)} 
      ...
      {currentStep === 'decision' && !loading && (
        <div style={{ textAlign: 'center', padding: '50px', background: '#f0f4f8', borderRadius: '15px', animation: 'fadeIn 0.5s' }}>
          <h2 style={{ fontSize: '2rem', marginBottom: '10px' }}>Quiz Finished! 🏁</h2>
          ...
          {weakTopics.length > 0 ? (
            <div style={{ background: '#f8d7da', padding: '20px', borderRadius: '12px', border: '1px solid #f5c2c7', maxWidth: '500px', margin: '0 auto 30px auto', textAlign: 'left' }}>
              ...
              <p style={{ color: '#842029' }}>The AI detected that you struggled with the following topics:</p>
              <ul style={{ color: '#842029', fontWeight: 'bold' }}>
                {weakTopics.map((topic, index) => (
                  <li key={index} style={{ marginBottom: '8px' }}>{topic.toUpperCase()}</li>
                ))}
              </ul>
              <p style={{ color: '#842029', fontSize: '0.95rem', marginBottom: 0 }}>
                Your upcoming adaptive session will dynamically generate a deep-dive review and new questions focused <strong>exclusively</strong> on these areas.
              </p>
            </div>
              ...
              <h3 style={{ color: '#0f5132', marginTop: 0 }}>🏆 Flawless Victory!</h3>
              <p style={{ color: '#0f5132', marginBottom: 0 }}>
                You answered everything correctly. Your adaptive session will feature advanced challenges to push your knowledge further!
              </p>
            </div>
          )}

          <div style={{ display: 'flex', gap: '15px', justifyContent: 'center' }}>
            <button onClick={startAdaptiveSession} style={{ padding: '15px 30px', background: '#007bff', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', fontSize: '1.1rem' }}>
              Start Adaptive Review 🚀
            </button>
            <button onClick={() => { window.location.reload() }} style={{ padding: '15px 30px', background: '#6c757d', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '1.1rem' }}>
              Quit Early
            </button>
          </div>
        </div>
      ...
      {currentStep === 'adaptive' && sessionData && !loading && (
        <div style={{ animation: 'fadeIn 0.5s' }}>
          ...
          <div style={{ background: '#fff3cd', padding: '25px', borderRadius: '12px', border: '1px solid #ffeeba', marginBottom: '20px' }}>
            ...
            <p style={{ lineHeight: '1.6', fontSize: '1.1rem', marginBottom: 0 }}>{sessionData.remediation_notes}</p>
          </div>
          ...
          {sessionData.targeted_quiz && sessionData.targeted_quiz.length > 0 && adaptiveAnsweredCount < sessionData.targeted_quiz.length ? (
            <div style={{ background: '#fff', padding: '20px', border: '1px solid #eee', borderRadius: '10px', boxShadow: '0 4px 6px rgba(0,0,0,0.05)' }}>
              <h3 style={{ borderBottom: '2px solid #eee', paddingBottom: '10px', color: '#333' }}>
                Mastery Check (Question {adaptiveAnsweredCount + 1} of {sessionData.targeted_quiz.length})
              </h3>
              
              <p style={{ fontSize: '1.2rem', fontWeight: 'bold', color: '#111' }}>
                {sessionData.targeted_quiz[adaptiveAnsweredCount].question}
              </p>
              ...
              {sessionData.targeted_quiz[adaptiveAnsweredCount].options?.map((opt, i) => {
                ...
                return (
                  <button 
                    key={i} 
                    disabled={adaptiveIsAnswerRevealed} 
                    onClick={() => handleAdaptiveAnswerSubmit(opt, sessionData.targeted_quiz[adaptiveAnsweredCount].correct_answer, sessionData.targeted_quiz[adaptiveAnsweredCount].topic_tag)} 
                    style={{ display: 'block', width: '100%', margin: '10px 0', padding: '15px', textAlign: 'left', borderRadius: '8px', border: `2px solid ${btnBorder}`, cursor: adaptiveIsAnswerRevealed ? 'default' : 'pointer', background: btnBg, fontSize: '1rem', color: '#111', fontWeight: '500' }}
                  >
                    {opt} {icon}
                  </button>
                )
              ...
              {adaptiveIsAnswerRevealed && (
                <button onClick={handleNextAdaptiveQuestion} style={{ width: '100%', padding: '15px', background: '#007bff', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '1.1rem', fontWeight: 'bold', marginTop: '15px' }}>
                  Next ➔
                </button>
              )}
            </div>
          ) : (
             <div style={{ textAlign: 'center', padding: '30px', background: '#d1e7dd', borderRadius: '10px', border: '1px solid #badbcc', display: (!sessionData.targeted_quiz || sessionData.targeted_quiz.length === 0 || adaptiveAnsweredCount >= sessionData.targeted_quiz.length) && weakTopics.length === 0 ? 'block' : 'none' }}>
               <h2 style={{ color: '#0f5132', margin: 0 }}>🎉 Review Phase Complete!</h2>
             </div>
          )}
          ...
          {(!sessionData.targeted_quiz || sessionData.targeted_quiz.length === 0 || adaptiveAnsweredCount >= sessionData.targeted_quiz.length) && (
            <div style={{ textAlign: 'center', marginTop: '30px' }}>
              {weakTopics.length > 0 ? (
                <div style={{ background: '#f8d7da', padding: '30px', borderRadius: '12px', border: '1px solid #f5c2c7' }}>
                  <h2 style={{ color: '#842029', margin: '0 0 10px 0' }}>Almost there! 🏋️</h2>
                  <p style={{ color: '#842029', fontSize: '1.1rem' }}>You still need to master: <strong>{weakTopics.join(', ')}</strong></p>
                  
                  <button 
                    onClick={startAdaptiveSession} 
                    style={{ width: '100%', maxWidth: '400px', padding: '15px', background: '#007bff', color: '#fff', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '1.1rem', fontWeight: 'bold', marginTop: '15px' }}
                  >
                    Generate Next Review Loop 🔄
                  </button>
                  <p style={{ marginTop: '15px' }}>
                    <button onClick={() => window.location.reload()} style={{ background: 'none', border: 'none', color: '#dc3545', textDecoration: 'underline', cursor: 'pointer' }}>I want to quit early</button>
                  </p>
                </div>
              ) : (
                <div style={{ background: '#d1e7dd', padding: '40px', borderRadius: '12px', border: '1px solid #badbcc' }}>
                  <h2 style={{ color: '#0f5132', fontSize: '2.2rem', margin: '0 0 10px 0' }}>🏆 Total Mastery Achieved!</h2>
                  <p style={{ color: '#0f5132', fontSize: '1.2rem', marginBottom: '30px' }}>You have successfully conquered all of your weak spots. The algorithm is satisfied.</p>
                  
                  <button 
                    onClick={() => { setCurrentStep('selection'); setInputType(null); setWeakTopics([]); }} 
                    style={{ padding: '15px 40px', background: '#198754', color: '#fff', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '1.2rem', fontWeight: 'bold' }}
                  >
                    Finish & Exit ➔
                  </button>
                </div>
...
const dropdownStyle = { flex: 1, padding: '12px', borderRadius: '8px', border: '1px solid #ccc', fontSize: '1rem', background: '#fff', color: '#111', cursor: 'pointer' }
